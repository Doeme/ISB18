...
\update{
Mortality is considered using a variable $\left[ \mu_{mort,j} \right] = \si{\milli\mole\per\gram_{DW}\per\hour}$ for each bacteria j in this implementation while Zhuang et al. modeled this
...
%			$\displaystyle b_{M,m} = v_{max,M,m} \; \frac{s_m}{k_{M,m}+s_m} \; \prod_{m'} \cfrac{1}{1+\cfrac{s_{m'}}{\mat{I}_{M,m,m'}}}$\;
    ...
    \ForEach{Model $M$ in models}{
      $\bio, \bm{v}$ \KwAssign $\mathrm {FBA}(M)$\;
      $\dot{x}_M = (\bio - \mu) \mul x_M$\;
