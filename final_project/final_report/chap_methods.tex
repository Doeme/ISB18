...
\begin{equation} \label{eq:diff_eq_x}
	\frac{\dd x_M}{\dd t} = (\mu_{g} - \mathrm{Mort}(\mu_{g})) x_M
\end{equation}
...
\update{Mortality is considered using a function $\mu_j(v_\bio)$ $\left[ \mu_{j} \right] = \si{\milli\mole\per\gram_{DW}\per\hour}$ for each bacteria $j$ in this implementation while Zhuang et al. modeled this
